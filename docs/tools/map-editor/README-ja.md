...
**落下配置**とは、ラウンド開始時にその場から落下し始める敵の配置方法です。
...
> テキスト入力欄にフォーカスがあるときはショートカットは無効です。  
> 各パレットボタンの右端にキーヒントが表示されます。

---

### 面のコピー

**📋 Copy to** ボタンとその右の入力欄を使います。
...
**💾 Save** ボタンで、現在表示中の面を VMAP\*\*\*.DAT としてダウンロードします。
...
**🗑 Clear** ボタンで、現在の面を全て道（スペース）に戻します。  
...
ツールバーの **↺ Undo** / **↻ Redo** ボタンをクリックするか、`Ctrl+Z` / `Ctrl+Y` で操作できます。

- 最大 **50手** の履歴をセッション中保持します
- 面をまたいだ操作も記録されます
- ブラウザを閉じると履歴は消えます
- 履歴がないときは Undo ボタンがグレーアウトします

---

### バリデーション（自動警告）

以下の条件に該当する場合、**ツールバー直下の警告バナー**に警告が表示されます。  
警告があっても保存は続行できます。
